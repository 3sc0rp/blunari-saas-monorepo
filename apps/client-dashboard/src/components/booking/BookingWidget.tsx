...
const BookingWidget: React.FC<BookingWidgetProps> = ({ slug, onError }) => {
  ...
  const handleBookingComplete = useCallback(
    async (reservation: { reservation_id: string; [key: string]: unknown }) => {
      if (!tenant) return;
      ...
      if (newAchievements.length > 0) {
        toast.success(
          `🎉 Achievement${newAchievements.length > 1 ? "s" : ""} unlocked: ${newAchievements.join(", ")}`,
        );
      }
    },
        ...
        <Card className="w-full max-w-md shadow-lg border-destructive/20">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-6">
              {tenantError ||
                "We couldn't load the restaurant information. This might be temporary."}
            </p>
                      ...
                      <motion.div
                        ...
                        animate={
                          step === state.step
                            ? {
                                boxShadow: [
                                  "0 0 0 0 rgba(var(--brand), 0.7)",
                                  "0 0 0 10px rgba(var(--brand), 0)",
                                ],
                              }
                            : {}
                        }
