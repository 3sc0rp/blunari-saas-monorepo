...
const BookingPatternsChart: React.FC<BookingPatternsChartProps> = ({
  data,
}) => {
  const [activeChart, setActiveChart] = useState<"hours" | "days" | "sources">(
    "hours",
  );
                ...
                {data.sourcePerformance.map((source, index) => (
                  <div key={source.source} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="text-body-sm font-medium text-text capitalize">
                        {source.source}
                      </div>
    ...
    switch (activeChart) {
      case "hours": {
        const peakHour = data.peakHours.reduce(
          ...
          data.peakHours[0] || { hour: 0, bookings: 0 },
        );
        return {
          title: "Peak Hour",
      ...
      case "days": {
        const peakDay = data.dayOfWeek.reduce(
          (max, current) => (current.bookings > max.bookings ? current : max),
          data.dayOfWeek[0] || { day: "Monday", bookings: 0 },
        );
        return {
          title: "Busiest Day",
      ...
      case "sources": {
        const topSource = data.sourcePerformance.reduce(
          (max, current) => (current.bookings > max.bookings ? current : max),
          data.sourcePerformance[0] || { source: "website", bookings: 0 },
        );
        return {
          title: "Top Source",
          value:
            topSource?.source?.charAt(0).toUpperCase() +
              topSource?.source?.slice(1) || "Website",
          ...
          <div className="flex rounded-md bg-surface-2 border border-surface-3">
            {chartTabs.map((tab) => (
              <Button
                ...
                size="sm"
                onClick={() =>
                  setActiveChart(tab.key as "hours" | "days" | "sources")
                }
      ...
      <CardContent className="pt-0">
        ...
        <div className="flex justify-start gap-6 mt-4 text-sm">
          ...
          {activeChart === "sources" && (
            <div className="flex items-center gap-2">
              <span className="text-text-muted">
                Distribution by booking source
              </span>
            </div>
          )}
        </div>
