      ...
      return {
        default: () => (
          <div className="flex flex-col items-center justify-center p-4 text-center">
            {FallbackIcon && <FallbackIcon className="w-8 h-8 text-amber-500 mb-2" />}
...
const SkeletonCard = memo(({ className, icon: Icon }: { 
  ...
  <div className={`bg-surface-2/30 animate-pulse rounded-lg flex items-center justify-center ${className}`}>
    {Icon && <Icon className="w-6 h-6 text-surface-2" />}
  </div>
));
...
const LoadingSpinner = memo(({ size = 8, className }: { 
  ...
  return (
    <div className={`flex items-center justify-center ${className}`}>
      <Loader2 className={`${sizeClass} animate-spin text-brand`} />
    </div>
  );
});
...
const CommandCenter: React.FC = memo(() => {
  ...
  return (
    <ErrorBoundary 
      ...
      fallback={
        <div className="flex flex-col items-center justify-center h-full">
          ...
          <h2 className="text-lg font-semibold text-text mb-2">Command Center Unavailable</h2>
          <p className="text-sm text-text-muted mb-4">We're having trouble loading the dashboard</p>
          <button
    ...
    >
      <div className="h-full flex flex-col bg-background" role="main" aria-label="Command Center Dashboard">
        {/* Onboarding Tour */}
        ...
        <header className="flex-shrink-0 border-b border-surface-2/30 bg-surface/80 backdrop-blur-sm">
          <div className="px-6 py-4">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-text">Command Center</h1>
                <p className="text-sm text-text-muted">Live operations dashboard</p>
              </div>
              <ErrorBoundary 
        ...
        <main className="flex-1 p-6 min-h-0" role="main">
          <div className="h-full grid grid-cols-12 gap-6">
            ...
            <section className="col-span-12 lg:col-span-7 flex flex-col min-h-0" aria-labelledby="timeline-header">
              <motion.div 
                className="flex-1 bg-surface rounded-xl border border-surface-2/50 shadow-elev-1 min-h-0"
              ...
              >
                <div className="h-full flex flex-col">
                  <div className="flex-shrink-0 p-4 border-b border-surface-2/30">
                    <h2 id="timeline-header" className="text-lg font-semibold text-text">Timeline</h2>
                    <p className="text-sm text-text-muted">Today's reservations and table status</p>
                  </div>
                    ...
                    <ErrorBoundary 
                      ...
                      fallback={
                        <div className="flex flex-col items-center justify-center h-full" role="alert">
                          <Clock className="w-8 h-8 text-amber-500 mb-2" aria-hidden="true" />
                          <p className="text-sm text-amber-600">Timeline temporarily unavailable</p>
                        </div>
                      }
                    >
            ...
            <div className="col-span-12 lg:col-span-5 flex flex-col gap-6 min-h-0">
              {/* Floor Map */}
              <motion.div 
                className="bg-surface rounded-xl border border-surface-2/50 shadow-elev-1 min-h-[300px] flex-1"
              ...
              >
                <div className="h-full flex flex-col">
                  <div className="flex-shrink-0 p-4 border-b border-surface-2/30">
                    <h2 className="text-lg font-semibold text-text">Floor Plan</h2>
                    <p className="text-sm text-text-muted">Interactive table layout</p>
                  </div>
                  <div className="flex-1 min-h-0 p-4">
                    ...
                    fallback={
                      <div className="flex flex-col items-center justify-center h-full">
                        <Square className="w-8 h-8 text-amber-500 mb-2" />
                        <p className="text-sm text-amber-600">Floor map temporarily unavailable</p>
                      </div>
                    }
                  >
              ...
              {/* Waitlist */}
              <motion.div 
                className="bg-surface rounded-xl border border-surface-2/50 shadow-elev-1 min-h-[200px] flex-1"
              ...
              >
                <div className="h-full flex flex-col">
                  <div className="flex-shrink-0 p-4 border-b border-surface-2/30">
                    <h2 className="text-lg font-semibold text-text">Waitlist</h2>
                    <p className="text-sm text-text-muted">Current waiting guests</p>
                  </div>
                  <div className="flex-1 min-h-0">
                    <ErrorBoundary 
                    maxRetries={2} 
                    ...
                    fallback={
                      <div className="flex flex-col items-center justify-center h-full">
                        ...
                        <p className="text-sm text-amber-600">Waitlist temporarily unavailable</p>
                      </div>
                    }
                  >
                      <Suspense fallback={<LoadingSpinner size={6} className="h-full" />}>
