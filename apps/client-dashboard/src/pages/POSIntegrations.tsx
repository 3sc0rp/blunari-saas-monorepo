...
const POSIntegrations: React.FC = () => {
  ...
  const [healthCheckLoading, setHealthCheckLoading] = useState<string | null>(
    null,
  );
  ...
  const IntegrationCard = ({ integration }: { integration: DbRecord }) => {
    const provider =
      providerInfo[integration.provider as keyof typeof providerInfo];
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-lg bg-gradient-to-br ${provider?.gradient} flex items-center justify-center text-white text-lg shadow-lg`}
              >
                {provider?.logo}
              </div>
              <div>
            ...
            <div className="flex justify-between text-sm">
              ...
              <span>
                {integration.last_health_check
                  ? new Date(integration.last_health_check).toLocaleDateString()
                  : "Never"}
              </span>
            </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent
              ...
              aria-describedby="pos-integration-description"
            >
              <div className="bg-gradient-to-br from-primary/5 via-background to-accent/5 p-6">
                <DialogHeader>
                  <p id="pos-integration-description" className="sr-only">
                    POS integration setup and configuration details
                  </p>
              ...
              <div className="p-6 pt-2">
                <div className="grid grid-cols-3 gap-6 max-w-4xl mx-auto">
                  {availableProviders.slice(0, 3).map((providerId, index) => {
                    const provider =
                      providerInfo[providerId as keyof typeof providerInfo];
                    return (
                      <motion.div
                        key={providerId}
                        ...
                        <Card className="group relative overflow-hidden border-2 border-transparent hover:border-primary/20 transition-all duration-300 hover:shadow-xl hover:shadow-primary/10 h-64 flex flex-col">
                          ...
                          <CardContent className="relative p-6 text-center space-y-4 flex-1 flex flex-col justify-between">
                            ...
                            >
                              <Button
                                onClick={() =>
                                  handleIntegrateProvider(providerId)
                                }
              ...
              <CardContent>
                <div className="space-y-3">
                  {integrations.slice(0, 5).map((integration) => {
                    const provider =
                      providerInfo[
                        integration.provider as keyof typeof providerInfo
                      ];
                    return (
                      <div
                        key={integration.id}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-6 h-6 rounded bg-gradient-to-br ${provider?.gradient} flex items-center justify-center text-white text-xs shadow-sm`}
                          >
                            {provider?.logo}
                          </div>
        ...
        <TabsContent value="events" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                All events received from POS systems
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
