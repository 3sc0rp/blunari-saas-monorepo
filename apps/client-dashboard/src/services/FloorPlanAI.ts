...
export class FloorPlanAI {
  ...
  static async analyzeFloorPlan(
    ...
    try {
      ...
      return {
        ...
        detectedTables: (result.detectedTables || []).map(
          (table: any, index: number) => ({
            ...
            position: {
              x: Math.max(
                0,
                Math.min(10, table.position?.x || Math.random() * 8 + 1),
              ),
              y: Math.max(
                0,
                Math.min(10, table.position?.y || Math.random() * 8 + 1),
              ),
            },
        ...
        recommendations: result.recommendations || [
          ...
          result.analysis ||
            "Advanced AI analysis of your restaurant floor plan",
          result.tableCount > 0
  ...
  private static async basicAnalysis(
    imageElement: HTMLImageElement,
    startTime: number,
  ): Promise<FloorPlanAnalysis> {
    ...
    const tableDetections = detections.filter(
      (detection: any) =>
        this.isTableLikeObject(detection.label) && detection.score > 0.3,
    );

    const detectedTables: DetectedTable[] = tableDetections.map(
    ...
    const averageConfidence =
      detectedTables.length > 0
        ? detectedTables.reduce((sum, table) => sum + table.confidence, 0) /
  ...
  private static generateRecommendations(
    ...
    if (tables.length === 0) {
      recommendations.push(
        "No tables detected. Try uploading a clearer floor plan image.",
      );
      recommendations.push(
        "Ensure tables are clearly visible and not obstructed by other objects.",
      );
    } else {
      ...
      if (lowConfidenceTables.length > 0) {
        recommendations.push(
          `${lowConfidenceTables.length} tables detected with low confidence. Manual verification recommended.`,
        );
      }

      const totalCapacity = tables.reduce(
        (sum, table) => sum + table.estimatedCapacity,
        0,
      );
      recommendations.push(
        `Estimated total seating capacity: ${totalCapacity} guests.`,
      );

      if (tables.length < 5) {
        recommendations.push(
          "Consider optimizing your layout for more tables if space allows.",
        );
      }
    }
