-- RPC to create catering package with SECURITY DEFINER, setting tenant_id from session
...
BEGIN
  tid := public.get_current_user_tenant_id();
  IF tid IS NULL THEN
...
COMMENT ON FUNCTION public.catering_create_package(jsonb) IS 'Creates a catering package for the current user''s tenant; bypasses RLS with SECURITY DEFINER and sets tenant_id automatically.';
