...
async function addDomain(supabase: any, params: any, cloudflareHeaders: any) {
  ...
  if (!cloudflareData.success) {
    ...
    return {
      ...
      warning: "Domain created but Cloudflare setup failed",
    };
  }
...
async function verifyDomain(
  ...
  if (!cloudflareData.success) {
    throw new Error(
      `Cloudflare error: ${cloudflareData.errors?.[0]?.message || "Unknown error"}`,
    );
  }
...
async function provisionSSL(
  ...
  if (!cloudflareData.success) {
    throw new Error(
      `Cloudflare SSL error: ${cloudflareData.errors?.[0]?.message || "Unknown error"}`,
    );
  }
