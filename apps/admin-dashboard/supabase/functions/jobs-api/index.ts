...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "cleanup") {
      ...
      try {
        ...
        const { data: deletedJobs, error: deleteError } = await supabaseClient
          ...
          .delete()
          .or(
            "job_type.eq.health_check,status.eq.failed,created_at.lt.2024-01-01",
          )
          .select();
        ...
        if (deleteError) {
          ...
          return new Response(
            JSON.stringify({
              error: "Failed to delete corrupted jobs",
    ...
    if (!response.ok) {
      ...
      console.error(
        `Background service error: ${response.status} - ${errorText}`,
      );
      return new Response(
        JSON.stringify({
