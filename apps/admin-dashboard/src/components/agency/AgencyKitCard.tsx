    ...
    <Card className="hover:shadow-md transition-shadow">
      <CardHeader className="pb-3">
        <div className="flex items-start gap-3">
          <div
            className={`p-2 rounded-lg ${getKitColor(kit.type)} bg-opacity-20`}
          >
            {getKitIcon(kit.type)}
          </div>
          ...
          <Badge className={getKitColor(kit.type)}>{kit.type}</Badge>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            Updated{" "}
            {formatDistanceToNow(new Date(kit.lastUpdated), {
              addSuffix: true,
            })}
          </span>
        </div>
