  ...
  const fetchAnalytics = async () => {
    try {
      ...
      try {
        ...
        if (tenantId) {
          ...
          const realAnalytics: CateringAnalytics = {
            ...
            average_order_value: Math.round(analyticsData.average_order_value || 0),
          ...
          for (let i = 5; i >= 0; i--) {
            ...
            monthlyRevenue.push({
              ...
              revenue: Math.floor(Math.random() * 50000) + (realAnalytics.total_revenue / 6), // Mock with some real data influence
              order_count: Math.floor(Math.random() * 20) + Math.floor(realAnalytics.total_orders / 6),
            });
          }
        ...
        console.warn('Comprehensive analytics function not available, falling back to mock data');
      }
      ...
      const mockAnalytics: CateringAnalytics = {
        ...
        popular_packages: [
          { package_id: 'pkg-1', package_name: 'Executive Business Package', order_count: 0 },
          { package_id: 'pkg-2', package_name: 'Luxury Wedding Package', order_count: 0 },
          { package_id: 'pkg-3', package_name: 'Cocktail Reception Package', order_count: 0 }
        ],
        popular_items: [
          { item_id: 'item-1', item_name: 'Smoked Salmon Canapés', order_count: 0 },
          { item_id: 'item-2', item_name: 'Herb-Crusted Rack of Lamb', order_count: 0 },
          { item_id: 'item-3', item_name: 'Chocolate Lava Cake', order_count: 0 }
        ],
        monthly_revenue: [],
