...
export const useEnhancedBackgroundOpsAPI = () => {
  ...
  const getJob = useCallback(
    async (id: string): Promise<BackgroundJob> => {
  ...
  const bulkCancelJobs = useCallback(
    async (
      jobIds: string[],
  ...
  const bulkRetryJobs = useCallback(
    async (
      jobIds: string[],
    ): Promise<{ successful: string[]; failed: string[] }> => {
      const results = { successful: [], failed: [] };
  ...
  const getJobHistory = useCallback(
    async (jobId?: string, limit = 100): Promise<JobHistoryEntry[]> => {
      return callAPI("jobs-api", {
  ...
  const logJobHistory = useCallback(
    async (
      jobId: string,
  ...
  const getMetrics = useCallback(
    async (
      type: "system" | "jobs" | "database" = "system",
    ): Promise<{
  ...
  const cleanupOldJobs = useCallback(
    async (olderThanDays = 30): Promise<{ cleaned: number }> => {
      return callAPI("jobs-api", {
  ...
  // Data Export
  const exportJobsData = useCallback(
    async (filters?: {
      status?: string;
      dateFrom?: string;
      dateTo?: string;
