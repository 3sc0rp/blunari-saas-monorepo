...
router.get('/orders', async (req, res): Promise<void> => {
  try {
    ...
    logger.info('Fetching catering orders', { tenantId, status, limit, offset });
    ...
    try {
      ...
      // Get total count
      let countQuery = 'SELECT COUNT(*) FROM catering_orders WHERE tenant_id = $1';
      ...
      if (dbError.code === '42P01' || dbError.message?.includes('does not exist')) {
        logger.warn('Catering tables not found, returning empty data', { tenantId });
        res.json({
          success: true,
          data: {
            orders: [],
...
router.post('/orders', async (req, res): Promise<void> => {
  try {
    ...
    try {
      ...
      // Handle case where catering tables don't exist yet
      if (dbError.code === '42P01' || dbError.message?.includes('does not exist')) {
        logger.warn('Catering tables not found, cannot create order', { tenantId });
        res.status(503).json({
          success: false,
...
router.get('/analytics', async (req, res): Promise<void> => {
  try {
    ...
    try {
      ...
      const { rows: popularPackages } = await db.query(packagesQuery, [tenantId]);
      ...
      const { rows: serviceTypes } = await db.query(serviceTypeQuery, [tenantId]);
      ...
      const conversionRate = totalInquiries > 0 ? (confirmedOrders / totalInquiries) * 100 : 0;
      ...
      res.json({
        ...
        data: {
          orders: {
            ...
            average_guest_count: Math.round((parseFloat(orders.average_guest_count) || 0) * 100) / 100
          },
          revenue: {
            ...
            average_order_value: Math.round((parseFloat(revenue.average_order_value) || 0) * 100) / 100,
      ...
      // Handle case where catering tables don't exist yet
      if (dbError.code === '42P01' || dbError.message?.includes('does not exist')) {
        logger.warn('Catering tables not found, returning empty analytics', { tenantId });
        res.json({
          success: true,
          data: {
            orders: {
