...
router.get('/', authenticateRequest, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    let filteredEvents = events.filter(event => event.tenantId === req.tenantId);
    ...
    if (aggregateId) {
      filteredEvents = filteredEvents.filter(event => event.aggregateId === aggregateId);
    }

    if (fromDate) {
      const from = new Date(fromDate as string);
      filteredEvents = filteredEvents.filter(event => 
        new Date(event.metadata.timestamp) >= from
      );
    }
    ...
    if (toDate) {
      ...
      filteredEvents = filteredEvents.filter(event => 
        new Date(event.metadata.timestamp) <= to
      );
    }
 ...
 */
router.get('/:id', authenticateRequest, async (req: AuthenticatedRequest, res) => {
  const requestLogger = logger.child({ requestId: req.requestId, tenantId: req.tenantId });
  
  try {
    const eventId = req.params.id;
    const event = events.find(e => e.id === eventId && e.tenantId === req.tenantId);
 ...
 */
router.get('/stream/:aggregateId', authenticateRequest, async (req: AuthenticatedRequest, res) => {
  const requestLogger = logger.child({ requestId: req.requestId, tenantId: req.tenantId });
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        currentVersion: aggregateEvents.length > 0 
