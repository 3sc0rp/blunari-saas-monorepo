...
router.get('/health/jobs-table', async (req, res) => {
  try {
    const result = await db.query('SELECT COUNT(*) as job_count FROM background_jobs');
    res.json({
      status: 'healthy',
...
router.get('/detailed', async (req, res) => {
  try {
    ...
    const isUnhealthy = Object.values(health.dependencies).some(status => status === 'unhealthy');
    if (isUnhealthy) {
