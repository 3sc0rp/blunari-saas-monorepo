...
export function validateSecurityConfig() {
  ...
  if (config.NODE_ENV === 'production') {
    ...
    if (!process.env.X_API_KEY || process.env.X_API_KEY.length < 32) {
      errors.push('X_API_KEY must be at least 32 characters long in production');
    }
    ...
    if (!process.env.SIGNING_SECRET || process.env.SIGNING_SECRET.length < 32) {
      errors.push('SIGNING_SECRET must be at least 32 characters long in production');
    }
    ...
    const defaultSecrets = ['your-secret-key', 'your-api-key', 'signing-secret'];
    if (defaultSecrets.some(secret => 
      config.JWT_SECRET.includes(secret) || 
      config.X_API_KEY.includes(secret) || 
      config.SIGNING_SECRET.includes(secret)
    )) {
      errors.push('Default secrets detected in production! Generate secure secrets immediately.');
    }
  }
  
  // General validations
  if (config.ALLOWED_ORIGINS.some(origin => origin.includes('localhost'))) {
    warnings.push('localhost origins detected - ensure this is intentional for your environment');
  }
  
  if (config.RATE_LIMIT_MAX_REQUESTS > 10000) {
    warnings.push('Very high rate limit detected - consider reducing for better security');
  }
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Security validation failed: ${errors.length} critical error(s) found`);
  }
