...
export function getActiveRequests(): Array<
  RequestMetrics & { id: string; duration: number }
> {
  const now = Date.now();
...
export async function getPerformanceMetrics(req: Request, res: Response) {
  try {
    ...
    const httpMetrics = currentMetrics.data.filter(
      (m: any) =>
        m.name.includes("http_request") || m.name.includes("http_requests"),
    );

    const responseTimeMetrics = httpMetrics.find((m: any) =>
      m.name.includes("duration"),
    );
    const requestCountMetrics = httpMetrics.find((m: any) =>
    ...
    res.json({
      ...
      summary: {
        ...
        slowRequests: activeRequests.filter((req) => req.duration > 1000)
          .length,
      },
...
async function getErrorRateMetrics(timeRange: string) {
  try {
    ...
    const errorMetrics = await metricsService.getMetricsByTags({
      status_class: "4xx",
    });
    const allMetrics = await metricsService.getAggregatedMetrics(
      "http_requests_total",
      timeRange,
    );
    ...
    return {
      ...
      aggregations: allMetrics.aggregations.map((agg: any) => ({
        ...
        error_rate:
          agg.sample_count > 0
            ? (errorMetrics.length / agg.sample_count) * 100
            : 0,
      })),
    };
  } catch (error) {
