...
export function adminAuditMiddleware(req: Request, res: Response, next: NextFunction) {
  const shouldLog = ADMIN_ACTIONS.has(req.method) || 
...
async function logAdminAction(req: Request, res: Response, responseData: any, duration: number) {
  try {
    ...
    await activityService.logActivity({
      ...
      status: res.statusCode < 400 ? 'success' : res.statusCode < 500 ? 'warning' : 'error',
      details: {
...
function sanitizeRequestBody(body: any): any {
  ...
  if (JSON.stringify(sanitized).length > 10000) {
    return { ...sanitized, _truncated: true, _originalSize: JSON.stringify(body).length };
  }
...
function getSeverityLevel(method: string, statusCode: number): 'low' | 'medium' | 'high' {
  ...
  try {
    ...
    res.json({
      ...
      summary: {
        ...
        criticalActions: auditLog.activities.filter(a => a.severity === 'high').length,
