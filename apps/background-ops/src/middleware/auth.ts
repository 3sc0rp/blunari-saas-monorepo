...
function verifyHmacSignature(payload: string, signature: string, secret: string): boolean {
  try {
    ...
    logger.warn('HMAC signature verification failed', { error: error instanceof Error ? error.message : 'Unknown error' });
    return false;
  }
}
...
export function authenticateRequest(req: AuthenticatedRequest, res: Response, next: NextFunction): void {
  ...
  logger.info('Authentication started', {
    requestId,
    tenantId: tenantId ? `tenant_${crypto.createHash('sha256').update(tenantId).digest('hex').slice(0, 8)}` : undefined,
    path: req.path,
    method: req.method,
    hasApiKey: !!apiKey,
