...
class SchedulerService {
  ...
  async initializeScheduler() {
    try {
      ...
      logger.info(`✅ Scheduler initialized with ${schedules.schedules.length} active schedules`);
    } catch (error) {
  ...
  async createSchedule(scheduleData: ScheduleData) {
    try {
      ...
      const id = uuidv4();
      const nextRun = this.calculateNextRun(scheduleData.schedule, scheduleData.timezone);
  ...
  async updateSchedule(scheduleId: string, updateData: Partial<ScheduleData>) {
    try {
      ...
      if (updateData.schedule) {
        ...
        const nextRun = this.calculateNextRun(updateData.schedule, updateData.timezone);
  ...
  async triggerSchedule(scheduleId: string) {
    try {
      ...
      logger.info(`Manually triggered schedule ${scheduleId}, created job ${job.id}`);
      return job;
    } catch (error) {
  ...
  async getScheduleHistory(scheduleId: string, options: { limit?: number; offset?: number } = {}) {
    try {
  ...
  async validateCronExpression(expression: string) {
    try {
      ...
      return {
        valid: false,
        error: error instanceof Error ? error.message : 'Unknown error validating cron expression'
      };
    }
  }
  ...
  private async startSchedule(schedule: any) {
    try {
      const task = cron.schedule(schedule.schedule_expression, async () => {
  ...
  private async executeSchedule(scheduleId: string) {
    try {
      ...
      // Update last run time and increment run count
      const nextRun = this.calculateNextRun(schedule.schedule_expression, schedule.timezone);
  ...
  private calculateNextRun(cronExpression: string, timezone: string = 'UTC', from?: Date): Date {
