...
async function broadcastToSubscribers(channel: string, dataProvider: () => Promise<any>) {
  try {
    ...
    logger.debug(`Broadcasted ${channel} update to ${subscribedClients.length} clients`);
  } catch (error) {
