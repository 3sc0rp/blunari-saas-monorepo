...
const commands = {
      ...
      const checks = [
        {
          ...
          command: "npm run type-check --workspaces",
        },
      ...
      const servers = [
        {
          name: "Backend API",
          command: "npm",
          args: ["run", "dev", "--workspace=@blunari/background-ops"],
        },
        {
          name: "Admin Dashboard",
          command: "npm",
          args: ["run", "dev", "--workspace=admin-dashboard"],
        },
        {
          name: "Client Dashboard",
          command: "npm",
          args: ["run", "dev", "--workspace=client-dashboard"],
        },
      ];
  ...
  "dev:backend": {
    ...
    action: async () => {
      ...
      await spawnCommand("npm", [
        "run",
        "dev",
        "--workspace=@blunari/background-ops",
      ]);
    },
  },
  ...
  "build:backend": {
    ...
    action: async () => {
      ...
      const result = executeCommand(
        "npm run build --workspace=@blunari/background-ops",
      );
  ...
  "test:e2e": {
    ...
    action: async () => {
      ...
      const result = executeCommand(
        "npm run test:e2e --workspace=client-dashboard",
      );
  ...
  "db:setup": {
    ...
    action: async () => {
      ...
      const supabaseCheck = executeCommand("supabase --version", {
        silent: true,
      });
  ...
  setup: {
    ...
    action: async () => {
      ...
      const qualityResult = executeCommand("npm run type-check --workspaces", {
        silent: true,
      });
      ...
      if (qualityResult.success) {
        ...
        log(
          "⚠️ Some quality issues found - run `npm run dev quality:fix` to fix",
          "yellow",
        );
      }
  ...
  status: {
    ...
    action: async () => {
      ...
      // Dependencies status
      log("\n📦 Dependencies:", "cyan");
      const outdated = executeCommand("npm outdated --depth=0", {
        silent: true,
      });
      ...
      // Build status
      log("\n🏗️ Build Status:", "cyan");
      const buildCheck = executeCommand("npm run type-check --workspaces", {
        silent: true,
      });
