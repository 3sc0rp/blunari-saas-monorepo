-- =============================================================================
-- FIX: provision_tenant Function - Add owner_id
-- =============================================================================
-- Issue: provision_tenant creates tenants without setting owner_id
-- Impact: manage-tenant-credentials cannot find tenant owner, tries to create duplicate
-- Solution: Add owner_id to tenant INSERT statement
-- =============================================================================
...
BEGIN
  ...
  -- 2. Create tenant WITH owner_id (CRITICAL FIX)
  INSERT INTO public.tenants (
    slug,
    name,
    owner_id,  -- ADDED: Link tenant to owner user
    status,
    timezone,
    currency,
    email,
  ...
  VALUES (
    p_tenant_data->>'slug',
    p_tenant_data->>'name',
    p_owner_user_id,  -- ADDED: Set owner_id from parameter
    'active',
    COALESCE(p_tenant_data->>'timezone', 'America/New_York'),
    COALESCE(p_tenant_data->>'currency', 'USD'),
    p_owner_email,
  ...
  RETURN jsonb_build_object(
    ...
    'message', 'Tenant provisioned successfully with owner linked'
  );

EXCEPTION WHEN OTHERS THEN
  -- Roll back is automatic in postgres for failed transactions
...
-- =============================================================================
-- Run this after creating a new tenant to verify owner_id is set:
-- SELECT id, name, slug, owner_id, email FROM tenants WHERE owner_id IS NOT NULL;
