-- Final corrected provision_tenant function with ONLY columns that actually exist
-- Based on actual schema inspection:
-- - tenants: name, slug, timezone, currency, status, description, phone, email, website, address
-- - profiles: email, first_name, last_name, phone, avatar_url, role, onboarding_completed, user_id
-- - employees: admin-only table, NOT used for tenant owners
...
BEGIN
  ...
  -- 2. Create or update profile for owner (with only columns that exist)
