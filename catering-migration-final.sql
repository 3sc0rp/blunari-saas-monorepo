-- Final Safe Catering Migration - Works with your actual tenant data
...
-- Create catering_packages table (without foreign key constraint to tenants)
CREATE TABLE IF NOT EXISTS public.catering_packages (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,  -- Remove FK constraint for now
  name TEXT NOT NULL,
  description TEXT,
...
-- Insert sample catering packages using the first available tenant
DO $$
DECLARE
    first_tenant_id UUID;
BEGIN
    -- Get the first tenant ID from your tenants table
    ...
    IF first_tenant_id IS NOT NULL THEN
        ...
        VALUES 
          (first_tenant_id, 'Executive Business Lunch', 'Professional catering for corporate meetings and business events. Includes gourmet sandwiches, salads, and beverages.', 2850, 10, true, true, false),
          (first_tenant_id, 'Wedding Reception Package', 'Elegant three-course dining experience perfect for wedding celebrations. Full service with setup and cleanup included.', 8500, 25, true, true, true),
          (first_tenant_id, 'Cocktail Party Package', 'Sophisticated appetizers and hors d''oeuvres for networking events and cocktail parties.', 4200, 15, false, true, false),
          (first_tenant_id, 'Holiday Celebration Menu', 'Festive catering for holiday parties and seasonal celebrations. Traditional favorites with a gourmet twist.', 5500, 20, false, false, false)
        ON CONFLICT (id) DO NOTHING;
        
        -- Also add the catering feature for this tenant
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tenant_features' AND table_schema = 'public') THEN
            IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tenant_features' AND column_name = 'config') THEN
                ...
                WHERE NOT EXISTS (
                  ...
                  WHERE tf.tenant_id = first_tenant_id AND tf.feature_key = 'catering'
                );
            ELSE
